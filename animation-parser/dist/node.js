...
// src/config.ts
var MODEL_VERSION = "ivone-intent-linear-int8-1.0.0";
...
function normalizeText(value) {
  return value.normalize("NFKC").replace(/[“”]/g, '"').replace(/[‘’]/g, "'").replace(/\*\*/g, "").toLowerCase().replace(/\s+/g, " ").trim();
}
...
function splitClauses(prompt) {
  return prompt.replace(/\b(at the same time|simultaneously)\b/gi, " while ").split(/[.!?;\n]+|,\s+(?=(?:then|after|before|while)\b)|\bthen\b/gi).map((part) => part.trim().replace(/^(?:and|then)\s+/i, "")).filter(Boolean);
}
function analyzePromptDelta(previousPrompt = "", currentPrompt) {
  ...
  return {
    changeType,
    summary: changeType === "unchanged" ? "Prompt is unchanged" : `${added.length} added, ${removed.length} removed, ${retained.length} retained`,
    addedClauses: added,
...
// src/entity-linker.ts
var PRONOUNS = /\b(it|its|they|them|their|he|him|his|she|her|this|that|the object|the character)\b/i;
function candidateNames(entity) {
  return [
    { value: entity.name, kind: "exact" },
    ...(entity.aliases ?? []).map((value) => ({ value, kind: "alias" })),
    { value: entity.type, kind: "type" }
  ];
}
function linkEntity(text, context) {
  ...
  const fuzzy = all.map((entity) => ({
    entity,
    score: Math.max(...candidateNames(entity).flatMap((name) => tokens.map((token) => similarity(token, name.value))))
...
var PRIMITIVES = { cube: "box", box: "box", sphere: "sphere", ball: "sphere" };
var CREATABLE_TYPES = ["box", "cube", "sphere", "ball", "character", "person", "robot", "door", "chair", "desk"];
var CLIPS = ["idle", "walk", "run", "crouch", "duck", "jump", "sit", "stand"];
var DIRECTIONS = ["lower-left", "lower-right", "upper-left", "upper-right", "left", "right", "up", "down", "forward", "backward", "back"];
function selectedEntity(input) {
  return input.selectedEntity ?? input.scene.entities.find((entity) => entity.id === input.selectedEntityId) ?? null;
}
function splitPromptSegments(prompt) {
  const normalizedConnectors = prompt.replace(/\b(at the same time|simultaneously)\b/gi, " while ").replace(/\bbefore\b/gi, " then ").replace(/\b(and then|after that|next)\b/gi, " then ");
...
function parseTiming(text, relation, defaults, kind) {
  const normalized = normalizeText(text);
...
function sceneEntityFromOperation(operation) {
  return {
...
function resolve(state, text, preferRecent = true) {
...
function positionAnchor(text, defaults, ground) {
  ...
  return xyz ? [Number(xyz[1]), Number(xyz[2]), Number(xyz[3])] : void 0;
}
function createEntity(state, text, modelCreateIntent = false) {
  const normalized = normalizeText(text);
  const typeWord = CREATABLE_TYPES.find((word) => new RegExp(`\\b${word}\\b`).test(normalized));
  const createIntent = /\b(add|create|make|spawn)\b/.test(normalized) || /^(?:a|an)\s+/.test(normalized) || /\b(put|place|starts?|begin)\b/.test(normalized) && /\b(?:a|an)\b/.test(normalized);
...
function applyModelIntentFallback(state, text, relation, prediction, entityRef) {
  if (prediction.confidence < 0.72 || prediction.label === "unsupported" || prediction.label === "create") return;
  const normalized = normalizeText(text);
  const timing = (kind) => parseTiming(text, relation, state.defaults, kind);
  ...
  switch (prediction.label) {
    ...
    case "rotate":
      makeAction(state, {
        type: "rotate",
        entityRef,
        target: { kind: "direction", direction: /\bright\b/.test(normalized) ? "right" : /\bleft\b/.test(normalized) ? "left" : void 0 },
    ...
    case "clip": {
      ...
      const clip = CLIPS.find((name) => new RegExp(`\\b${name}(?:s|es|ing)?\\b`).test(normalized)) ?? Object.entries(clipAliases).find(([alias]) => new RegExp(`\\b${alias}(?:s|ed|ing)?\\b`).test(normalized))?.[1];
      if (clip) {
        makeAction(state, {
          type: "clip",
          entityRef,
    ...
    case "fall":
      ...
      break;
    case "interact": {
      const interaction = /\b(close|shut|seal)\b/.test(normalized) ? "close" : /\b(open|unseal|unlatch)\b/.test(normalized) ? "open" : null;
      if (interaction) {
        makeAction(state, {
          type: "interact",
          entityRef,
...
function parseSegment(state, text, relation, prediction) {
  const actionCountBefore = state.actions.length;
  const entityCountBefore = state.entities.length;
  const normalized = normalizeText(text).replace(/\bcamra\b/g, "camera").replace(/\bmvoe\b/g, "move").replace(/\brihgt\b/g, "right");
  ...
  const common = (kind) => parseTiming(text, relation, state.defaults, kind);
  ...
  if ((/\bcamera\b/.test(normalized) || /\bframe\b/.test(normalized)) && camera) {
    ...
    if (/\b(follow|follows|following|track|tracks|tracking)\b/.test(normalized)) {
      makeAction(state, {
        ...
        timing: common("cameraTrack"),
        values: { targetReference: targetRef },
        sourceText: text,
        confidence: targetRef.entityId ? 0.95 : 0.35
      });
      return;
    }
    ...
    if (/\b(move|dolly|pan|zoom)\b/.test(normalized)) {
      ...
      makeAction(state, {
        type: "cameraMove",
        entityRef,
        target: direction ? { kind: "direction", direction } : void 0,
        timing: common("cameraMove"),
        relative: true,
        sourceText: text,
        confidence: direction ? 0.92 : 0.68
      });
      return;
    }
  }
  const locomotionWithDestination = /\b(walk|walks|run|runs)\b/.test(normalized) && /\b(left|right|up|down|forward|back|backward|to|toward|towards)\b/.test(normalized);
  if (/\b(move|moves|slide|slides|scoot|scoots|rush|rushes|head|heads|go|goes|travel|travels)\b/.test(normalized) || locomotionWithDestination) {
    ...
    makeAction(state, {
      type: "move",
      entityRef,
      target: targetRef?.entityId ? { kind: "entity", entityId: targetRef.entityId } : { kind: "direction", direction: direction ?? "forward", amount: distanceMatch ? Number(distanceMatch[1]) : void 0, unit: distanceMatch?.[2] },
      timing,
  ...
  if (/\b(face|faces|look at|looks at)\b/.test(normalized) && !/\bcamera\b/.test(normalized)) {
    ...
    makeAction(state, {
      ...
      values: { targetReference: targetRef },
      sourceText: text,
      confidence: entityRef.entityId && targetRef.entityId ? 0.92 : 0.3
    });
  }
  if (/\b(scale|scales|grow|grows|shrink|shrinks|bigger|smaller)\b/.test(normalized)) {
    ...
    makeAction(state, {
      ...
      entityRef,
      timing: common("scale"),
      relative: !/\bto\b/.test(normalized),
      values: { amount },
      sourceText: text,
      confidence: entityRef.entityId ? 0.95 : 0.2
    });
  }
  const colorName = Object.keys(COLORS).find((color) => new RegExp(`\\b(?:turns?|becomes?|change(?:s|d)?(?:\\s+\\w+)?\\s+to|paint(?:s|ed)?(?:\\s+(?:it|the\\s+\\w+))?|colour|color)\\s+${color}\\b`).test(normalized));
  if (colorName) {
    makeAction(state, {
      type: "appearance",
      entityRef,
      timing: common("appearance"),
      values: { color: COLORS[colorName], colorName },
      sourceText: text,
      confidence: entityRef.entityId ? 0.98 : 0.2
    });
  }
  ...
  if (/\b(open|opens|close|closes)\b/.test(normalized)) {
    ...
    makeAction(state, {
      type: "interact",
      entityRef,
      timing: common("interact"),
      values: { interaction: verb },
      sourceText: text,
      confidence: entityRef.entityId ? 0.96 : 0.2
    });
  }
  ...
  if (keyframe && /\b(position|rotation|scale|keyframe)\b/.test(normalized)) {
    makeAction(state, {
      type: "keyframe",
      entityRef,
      timing: { start: Number(keyframe[1]), duration: 0, relation },
      relative: /\brelative\b/.test(normalized),
      values: { mode: /\brelative\b/.test(normalized) ? "relative" : "absolute" },
      sourceText: text,
      confidence: entityRef.entityId ? 0.9 : 0.2
    });
  }
...
function collectIssues(state) {
  for (const action of state.actions) {
    ...
    const targetRef = action.values?.targetReference;
...
function parseAnimationPrompt(input, defaults = DEFAULT_SEMANTICS, modelPredictions = []) {
...
${JSON.stringify(defaults)}`);
  ...
  state.actions.forEach((action, index) => {
    ...
    action.timing.duration = action.timing.duration === void 0 ? void 0 : clamp(action.timing.duration, 0, defaults.maximumTimelineDuration);
  });
  collectIssues(state);
  ...
  const unresolvedReferences = state.actions.flatMap((action) => {
    const refs = [action.entityRef, action.values?.targetReference].filter(Boolean);
  ...
  const confidence = !currentPrompt ? 0 : actionable.length || state.entities.length ? clamp((actionable.reduce((sum, action) => sum + action.confidence, 0) + state.entities.reduce((sum, entity) => sum + entity.confidence, 0)) / Math.max(1, actionable.length + state.entities.length) - unresolvedReferences.length * 0.08, 0, 1) : 0.1;
  return {
...
function push(state, action, type, entityId, fields = {}) {
...
function resolveOne(state, action, suffix = "") {
  ...
  switch (action.type) {
    ...
    case "cameraMove": {
      ...
      if (action.target?.kind === "entity" && action.target.entityId) {
        ...
        const style2 = action.timing.style ?? "normal";
        const semantic2 = state.defaults.movementStyles[style2] ?? state.defaults.movementStyles.normal;
        const amount = action.target?.amount ?? (style2 === "normal" ? state.defaults.directionDistance : semantic2.speed * (action.timing.duration ?? 1));
        const directions = action.values?.directions ?? [action.target?.direction ?? "forward"];
    ...
    case "clip": {
      ...
      push(state, action, canonical, entityId, { clip, locomotion: ["walk", "run"].includes(clip) ? clip : void 0 });
      break;
    }
    case "cameraTrack":
...
function resolveToPlanner(result, scene, defaults = DEFAULT_SEMANTICS) {
  ...
  const operations = result.entities.map((entity) => {
...
var animation_ir_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://ivone.dev/schemas/animation-ir/1.0.json",
  title: "Ivone deterministic animation intermediate representation",
  type: "object",
  additionalProperties: false,
  required: ["schemaVersion", "modelVersion", "promptRevision", "entities", "actions", "unresolvedReferences", "warnings", "confidence", "deterministicKey"],
  properties: {
    schemaVersion: { const: "1.0" },
    modelVersion: { type: "string", minLength: 1 },
    deterministicKey: { type: "string", pattern: "^[a-f0-9]{16}$" },
    confidence: { type: "number", minimum: 0, maximum: 1 },
    promptRevision: {
      type: "object",
      additionalProperties: false,
      required: ["changeType", "summary", "addedClauses", "removedClauses", "retainedClauses"],
      properties: {
        changeType: { enum: ["initial", "add", "modify", "remove", "unchanged"] },
        summary: { type: "string" },
        addedClauses: { $ref: "#/$defs/stringArray" },
        removedClauses: { $ref: "#/$defs/stringArray" },
        retainedClauses: { $ref: "#/$defs/stringArray" }
      }
    },
    entities: {
      type: "array",
      items: {
        type: "object",
        additionalProperties: false,
        required: ["operation", "entityId", "sourceText", "confidence"],
        properties: {
          operation: { enum: ["create", "modify", "remove"] },
    ...
    unresolvedReferences: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["text", "candidates", "reason"],
        properties: {
          text: { type: "string" },
          actionId: { type: "string" },
          candidates: { $ref: "#/$defs/stringArray" },
          reason: { type: "string" }
        }
      }
    },
  ...
  $defs: {
    ...
    entityReference: {
      type: "object",
      additionalProperties: false,
      required: ["text", "resolution", "confidence"],
      properties: {
        text: { type: "string" },
        entityId: { type: "string" },
        resolution: { enum: ["exact", "alias", "type", "selected", "pronoun", "created", "unresolved"] },
        confidence: { $ref: "#/$defs/confidence" },
        candidates: { $ref: "#/$defs/stringArray" }
      }
    },
    timing: {
      ...
      properties: {
        ...
        style: { type: "string" },
        easing: { type: "string" },
        relation: { enum: ["sequence", "parallel", "before", "after"] },
        relativeToActionId: { type: "string" }
      }
    },
    spatialTarget: {
      type: "object",
      ...
      required: ["kind"],
      properties: {
        kind: { enum: ["position", "direction", "entity", "relationship", "ground"] },
    ...
    action: {
      type: "object",
      additionalProperties: false,
      required: ["id", "type", "entityRef", "timing", "sourceText", "confidence"],
      properties: {
        id: { type: "string", minLength: 1 },
        type: { enum: ["place", "move", "rotate", "face", "scale", "appearance", "clip", "cameraMove", "cameraTrack", "cameraFocus", "interact", "fall", "keyframe", "wait", "repeat", "remove"] },
        entityRef: { $ref: "#/$defs/entityReference" },
        target: { $ref: "#/$defs/spatialTarget" },
...
var validateSchema = ajv.compile(animation_ir_schema_default);
...
function safeRepairParseResult(result) {
  ...
  const actions = result.actions.filter((action) => {
    ...
    if (action.timing.start !== void 0) action.timing.start = Math.max(0, Number(action.timing.start) || 0);
    if (action.timing.duration !== void 0) action.timing.duration = Math.max(0, Number(action.timing.duration) || 0);
...
// src/runtime.ts
var BrowserAnimationParser = class {
  worker = null;
  progress = { phase: "idle", loaded: 0, message: "Not initialized" };
  sequence = 0;
  ...
  async start() {
    ...
    const resolvedBaseUrl = typeof location !== "undefined" ? new URL(this.baseUrl, location.href).href : this.baseUrl;
...
var animationParser = new BrowserAnimationParser();

// src/planner-adapter.ts
var PlannerAnimationParser = class {
  constructor(parser = new BrowserAnimationParser(), defaults = DEFAULT_SEMANTICS) {
    this.parser = parser;
    this.defaults = defaults;
  }
  initialize() {
    return this.parser.initialize();
  }
  getLoadProgress() {
    return this.parser.getLoadProgress();
  }
  getModelVersion() {
    return this.parser.getModelVersion();
  }
  isWebGPUSupported() {
    return this.parser.isWebGPUSupported();
  }
  dispose() {
    this.parser.dispose();
  }
  async parseToPlanner(input) {
